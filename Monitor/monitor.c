...
CPU_registers_cache CPU_Registers_Cache;
registerP registers[WATCH_MENU_ROWS_MAX];
//---------------------------------
//Functions
/**************************************************
* Func: reg_init                                  *
...
void reg_init() {
	if (hMapFile = OpenFileMapping(FILE_MAP_ALL_ACCESS, FALSE, "Global\\SharedMemory"))
		CPU_Registers = (CPU_registers *) MapViewOfFile(hMapFile, FILE_MAP_ALL_ACCESS, 0, 0, sizeof(CPU_registers));
	else {
...
void regWatch_uninit(int index) {
	...
	if (index == reg_stack_top) //Do nothing
		...
		mem_shiftback(registers, sizeof(registerP), index, reg_stack_top); //Shift the stack backwards
}
/**************************************************************
* Func: texture_init                                          *
...
*                                                   *
****************************************************/
void gui_shutdown(struct nk_glfw *glfw, GLFWwindow *main_win) {
...
int main() {
	...
	//---------------------------------
	//GUI Infinite loop :P
	while(!glfwWindowShouldClose(main_win)) { //Close button
